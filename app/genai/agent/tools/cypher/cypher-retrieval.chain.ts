...
import initGenerateAuthoritativeAnswerChain from "./authoritative-answer.chain";
...
async function recursivelyEvaluate(
  ...
  const exampleCypher = `
   Fixable Example #1:
    * cypher:
        MATCH (a:Ingredient {{name: 'Copper'}})<-[:HAS_INGREDIENT]-(s:Supplement)
        RETURN a.name AS Ingredient, s.name AS Supplement, elementId(s) AS _id, r.role AS Role
        LIMIT 10
    * errors: ["Variable \`r\` not defined (line 1, column 172 (offset: 171))"]
    * response:
        MATCH (a:Ingredient {name: 'Copper'})<-[r:HAS_INGREDIENT]-(s:Supplement)
        RETURN a.name AS Ingredient, s.name AS Supplement, elementId(s) AS _id, r.role AS Role
        LIMIT 10
  `;
  ...
  while (results === undefined && retries < 5) {
    try {
      ...
      const evaluation = await evaluationChain.invoke({
        ...
        schema: graph.getSchema(),
        errors: [e.message],
        indexes: await graph.query("SHOW INDEXES"),
        exampleCypher: `
         Fixable Example #1:
    * cypher:
        MATCH (a:Ingredient {{name: 'Copper'}})<-[:HAS_INGREDIENT]-(s:Supplement)
