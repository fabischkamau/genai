...
describe("Conversation History", () => {
  ...
  it("should save conversation history", async () => {
    ...
    const source = "cypher";
    const input = "Which supplimets are best for testesterone";
    const output = "The Best suppliment for testesterone";
    const rephrasedQuestion = "Which supplimets are best for testesterone";
    const cypher = `MATCH (p:Product {name:Macular Support with Lutein, Zeaxanthin & Zinc (60 Vegetarian Capsules)}) RETURN p.name as name`;

    // Save message
    const id = await saveHistory(sessionId, input, output, ids, cypher);
